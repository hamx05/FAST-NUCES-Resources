...
def login():
    ...
    if verify_credentials(username, password):
        login_window.destroy()  # Destroy login window after login
        manage_window(username)  # Open manage window
    else:
...
def manage_window(username):
    ...
    # Create UI elements for managing site data
    ...
    tk.Button(manage_window, text="Save Site", command=lambda: save_site_data(username, site_name_entry.get(), site_url_entry.get(), site_username_entry.get(), site_password_entry.get())).grid(row=4, column=0, pady=10)
    
    # Display existing sites if any
    existing_sites = load_site_data(username)
    for idx, site in enumerate(existing_sites):
        tk.Label(manage_window, text=f"{site['site_name']} - {site['site_username']}").grid(row=5+idx, column=0, padx=10, pady=10)
