...
def view_credentials():
    ...
    private_keys = readPrivateKeys()
    if len(private_keys) != len(credentials):
        messagebox.showerror("Error", "Mismatch between credentials and private keys.")
        return

    credential_list = "\n".join(
        f"{i + 1}. Website: {cred['website']}, Username: {cred['username']}, Password: {RSAdecrypt(cred['password'], (private_keys[i], cred['modulus']))}"
        for i, cred in enumerate(credentials)
    )
    messagebox.showinfo("Stored Credentials", credential_list)
