...
def loadCredentials(username: str) -> list:
    """
    Loads the credentials for the given user from the credentials file.

    Args:
        username (str): The username whose credentials to load.

    Returns:
        list: A list of credentials for the user, or an empty list if the user doesn't exist.
    """
    try:
        # Open the user's credentials file
        with open(f"{username}_credentials.json", "r") as file:
            return json.load(file)
    except FileNotFoundError:
        # If file doesn't exist for the user, return an empty list
        return []
...
def isUserValid(username: str, password: str) -> bool:
    ...
    user_data = loadCredentials()

    # Check if username exists and password matches
    if username in user_data and user_data[username] == md5(password):
        return True
    return False
...
def analyzePasswords() -> str:
    """
    Analyze the passwords stored for the current user.

    Returns:
        str: A summary of password strengths for the user.
    """
    credentials = loadCredentials(CURRENT_USER)  # Load the credentials for the current user

    if not credentials:
        return "No credentials found for the current user."

    # Analyze the credentials (passwords) and return a summary (for example)
    strength_summary = ""
    for credential in credentials:
        password = credential.get("password")
        strength_summary += f"Password for {credential.get('website')}: {validatePasskey(password)}\n"
...
def storeLoginCredentials(user_data: dict) -> None:
    """
    Store the login credentials (username and password hash) in the credentials file.

    Args:
        user_data (dict): A dictionary containing the username and password hash.
    """
    try:
        # Load existing data or initialize empty dictionary
        credentials_file = f"{user_data['username']}_credentials.json"
        if os.path.exists(credentials_file):
        ...
        # Add new credential (e.g., username and hashed password)
        new_credential = {
        ...
        messagebox.showerror("Error", f"Failed to store credentials: {e}")
