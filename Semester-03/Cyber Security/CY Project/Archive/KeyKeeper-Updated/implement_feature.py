...
def appendCredential(website: str, username: str, password: str, public_key: tuple[int], private_key: tuple[int]) -> None:
    """
    This function takes the new credential details as input, creates a new dictionary/object,
    appends it to the list, and saves the updated list.
    """
    try:
        # Extract encryption keys and modulus
        ...
        if credentials_list is None:
            raise ValueError("Failed to load credentials. Cannot append new credential.")
        ...
        # Append the new credential to the credentials list
        credentials_list.append(new_credential)

        # Save the updated credentials list back to the file
        saveCredentials(credentials_list)

    except Exception as e:
        print(f"Error: An unexpected error occurred while appending the credential: {e}")

def deleteCredential(index: int) -> None:
    """
    This function takes the index of the credential to be deleted, removes it from the list,
    and saves the updated list.
    """
    try:
        # Load existing credentials
        credentials_list = loadCredentials(CURRENT_USER)
        if credentials_list is None or len(credentials_list) == 0:
            raise IndexError("No credentials to delete.")

        # Check if the index is valid
        if index < 0 or index >= len(credentials_list):
            raise IndexError(f"Invalid index: {index}. Please select a valid credential.")

        # Remove the credential at the given index
        del credentials_list[index]

        # Save the updated list back to the file
        saveCredentials(credentials_list)
        print(f"Success: Credential at index {index} deleted successfully.")

    except IndexError as e:
        print(f"Error: {e}")
    except Exception as e:
        print(f"Error: An unexpected error occurred while deleting the credential: {e}")
...
#     setup_password_window(username)
...
def setup_user() -> None:
    ...
    # Password strength meter label and progress bar
    ...
    def submit_user():
        try:
            ...
            # Hash the password (assuming hash_password function is defined)
            ...
            # Store the user data (username and hashed password) in the file
            storeLoginCredentials(user_data)

            print(f"User '{username}' added successfully.")

            # Now load credentials for the current user
            credentials = loadCredentials(CURRENT_USER)  # No need to pass username
            print(f"Credentials for {CURRENT_USER}: {credentials}")

        except Exception as e:
            print(f"Error submitting user: {e}")

    submit_button = tk.Button(user_window, text="Create User", command=submit_user)
    submit_button.pack(pady=10)

    # Update the strength meter in real-time as the user types
...
def setup_password_window(username: str) -> None:
    """Handles password setup in a separate window."""
    password_window = tk.Toplevel(root)
    password_window.title("Set Your Password")

    # Create the password input field and strength label
    password_label = tk.Label(password_window, text="Set your password:")
    password_label.pack(pady=5)
    ...
    strength_label = tk.Label(password_window, text="Strength: Not checked")
    strength_label.pack(pady=5)
    
    progress = ttk.Progressbar(password_window, style="danger.Horizontal.TProgressbar", length=200, mode='determinate')
    progress.pack(pady=5)
    ...
    def validate_and_store_user():
        ...
        try:
            ...
            password_window.quit()  # Close the password setup window
    ...
    submit_button = tk.Button(password_window, text="Submit", command=validate_and_store_user)
    submit_button.pack(pady=10)
