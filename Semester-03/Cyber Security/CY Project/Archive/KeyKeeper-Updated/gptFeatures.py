...
def setup_user() -> None:
    ...
    # After username entry, open a new window to set the password
...
def setup_password_window(username: str) -> None:
    ...
    # Function to update the strength meter when the user types the password
    ...
    password_entry.bind("<KeyRelease>", on_password_change)  # Bind the key release event to update the strength

    def validate_and_store_user():
        password = password_entry.get()
        
        # Validate the password strength using the updated function
        is_valid, reason = validatePasskey(password)
        ...
        try:
            ...
            show_main_interface()  # Go back to the main login screen
        except Exception as e:
            messagebox.showerror("Error", f"Failed to set up user: {e}")
...
#     # Password entry for setting new password (if needed)
#     password_label = tk.Label(main_window, text="Set your password:")
...
#     strength_label = tk.Label(main_window, text="Password strength: ", font=("Arial", 12))
#     strength_label.pack(pady=5)

#     # Progress bar to show password strength
#     strength_meter = tk.Progressbar(main_window, orient="horizontal", length=200, mode="determinate")
#     strength_meter.pack(pady=10)

#     # Update strength meter as user types the password
#     password_entry.bind("<KeyRelease>", lambda event: update_strength_meter(password_entry.get(), strength_label, strength_meter))
...
def add_credential() -> None:
    ...
    # Create the strength label and progress bar for the strength meter
    strength_label = tk.Label(root, text="Strength: Not checked")
    strength_label.pack(pady=5)
    progress = ttk.Progressbar(root, style="danger.Horizontal.TProgressbar", length=200, mode='determinate')
    progress.pack(pady=5)
    ...
    # Wait until user confirms the password (e.g., by clicking OK)
    def submit_credentials():
        password = password_entry.get()  # Get the final password from the entry widget

        # Proceed only if password is valid
        passwordStrength = checkPasswordStrength(password)
        if "Weak" in passwordStrength:
            ...
            root.quit()  # Close the root window or transition to another part of the app

    # Add a button to submit the credential
...
if __name__ == "__main__":
    ...
    root = tk.Tk()
    root.title("Login - Password Manager")
