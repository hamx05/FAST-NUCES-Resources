...
if __name__ == "__main__":
    ...
    try:       
        ...
        # User Access
        if not EXIT_FLAG and ACCESS_FLAG:

            print("*** ALERT: Access granted.")

            # Generate RSA keys
            public_key, private_key = generateRSAkeys() # move this above appendCredentials when keys become dynamic

            # note: everything below is testing code
            
            mockData = [ 
                ['facebook', 'hamza@email.com', 'hamzaPW123'],
                ['instragram', 'hamz_', 'hamzaPW'],
                ['twitter', 'hamza_', 'hamzaPW.'],
                ['discord', 'hxmz', 'hamzaPW..'],
            ]

            for data in mockData:
                appendCredential(data[0], data[1], RSAencrypt(data[2], public_key), public_key, private_key)

            ListA = loadCredentials()
            for a in ListA:
                print(a)
            ...
            for i in range(len(ListA)):
                print(ListA[i], RSAdecrypt(ListA[i]['password'], private_key))
