...
// Attempt to tokenize the decrypted text to form valid words
vector<string> tokenizeText(const string& text, const vector<string>& dictionary) {
    vector<string> tokens;
    string currentToken;

    for (int i = 0; i < text.length(); ++i) {
        currentToken += text[i]; // Add current character to the token
        ...
        if (isWordInDictionary(currentToken, dictionary)) {
            ...
            currentToken = ""; // Start a new token
        }
    }
...
int main() {
    ...
    for (int key = 0; key < 26; key++) {
        vector<string> tokens = tokenizeText(convertedTexts[key], dictionary); // Tokenize the decrypted text
